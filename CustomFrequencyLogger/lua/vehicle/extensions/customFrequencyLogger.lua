...
-- Modified for high frequency logging (10Hz+)
-- Merged with original data collection modules for full feature support
...
-- ==========================================
--  Original Data Collection Modules Start
--  (Copied from original file for completeness)
-- ==========================================
...
-- ==========================================
--  Original Data Collection Modules End
-- ==========================================
...
local function initOutput()
  for _, moduleID in pairs(moduleIdx) do
    ...
    outputStreams[moduleID] = { txt = "", fpath = fpath }
  end
end
...
local function initSettings()
  -- 【修改点】 默认输出目录改为 CustomLog
  settings.outputDir = "CustomLog"
  -- 【修改点】 默认采样率为 0.1 (10Hz)
  settings.updatePeriod = 0.1
...
local function applySettingsFromJSON(fpath)
  ...
  if s.updatePeriod ~= nil then settings.updatePeriod = s.updatePeriod end
  if s.outputDir ~= nil then settings.outputDir = s.outputDir end
  ...
  for _, moduleID in pairs(moduleIdx) do
    ...
    for statID, stat in ipairs(record[moduleID]) do
      if s.useStat and s.useStat[moduleID] then
...
local function onExtensionLoaded()
  ...
  initSettings()
  -- 触发自定义的 hook，以免和原版冲突
  guihooks.trigger("LoadedCustomFrequencyLogger")
end
...
local function updateGFX(dt)
  ...
  -- 原版是 1024 步存一次。
  -- 10Hz 下 1024 步大约是 100秒，这个频率写文件是可以接受的。
...
local function suggestOutputFilename()
  local get_fname = function(i)
    return "custom_vehicle_stats_settings(" .. string.format("%03d", i) .. ").json"
  end
  local fn = settings.outputDir .. "\\" .. "custom_vehicle_stats_settings.json"
  local i = 1
