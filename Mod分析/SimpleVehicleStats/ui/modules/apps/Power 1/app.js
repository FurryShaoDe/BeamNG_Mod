...
.directive('power1', [function () {
  return {
    template: '<div><power1temp></power1temp></div>',
    replace: true,
    restrict: 'EA',
    link: function (scope, element) {
      ...
      vueApp.component('power1temp', {
        ...
        mounted() {
          ...
          const updatePower = () => {
            bngApi.activeObjectLua(LuaPower, (ps) => {
              const hp = Math.max(0, Math.ceil((ps || 0) * 0.986))
              this.powerHP = hp
              this.updateRatio()
            })
          }
          ...
          updatePower()
          scope.$on('VehicleFocusChanged', updatePower)
