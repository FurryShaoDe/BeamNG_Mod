...
实际承载应用的 DOM 元素。`domElement` 由指令名称决定，本质上是从 *camelCase* 转换为 *lisp-case*，因此在本例中是从 `myApp` 转换为 `<my-app></my-app>`
...
应用首次启动时使用的默认 CSS 属性，包括宽度、高度，以及 top/bottom、left/right 属性，这些属性还决定了应用将对齐到屏幕的哪个角落
